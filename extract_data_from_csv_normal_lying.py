...
for row in df.iterrows():
	...
	normal_channel = row['normal channel']
	if pd.isnull(normal_channel):
		continue

	normal_start = row['normal start']
	normal_end = row['normal end']
	if type(normal_start) == float:
		...
		normal_start = int(row['normal start'] * 60 * sample_frequency)
	...
	if type(normal_end) == float:
		...
		normal_end = int(row['normal end'] * 60 * sample_frequency)
	...
	file_path = os.path.join("sliced_data_lying/NORMAL/", filename + '.csv')
	...
	plt.savefig("sliced_data_lying/NORMAL/" + filename + ".png")
	plt.close("all")